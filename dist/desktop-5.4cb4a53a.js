function vacuumWorld(locationInput, statusInputs) {
    ...
    function moveToRoom(roomIndex) {
        ...
        if (roomIndex === locationInput) output += "No action. Already in Room " + (roomIndex + 1) + ".";
        else {
    ...
    for(let i = 0; i < numRooms; i++)if (statusInputs[i] === 1) cleanRoom(i);
    // Add status of all rooms to output
    output += "<p>Current Room Status:</p>";
    for(let i = 0; i < numRooms; i++)output += "<p>Room " + (i + 1) + ": " + (goalState[i] === 1 ? "Dirty" : "Clean") + "</p>";
...
//# sourceMappingURL=desktop-5.4cb4a53a.js.map
